...
if [[ "$1" == "record" ]]; then
    ...
    python -m lerobot.record \
        ...
        --robot.id=follower \
        --robot.cameras="{ front: {type: opencv, index_or_path: 3, width: 1280, height: 720, fps: 30}}" \
        --robot.cameras="{ front: {type: opencv, index_or_path: 6, width: 1280, height: 720, fps: 30}}" \
